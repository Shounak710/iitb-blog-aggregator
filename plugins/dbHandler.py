...
# publication date and last updated is not yet implemented
...
def dbHandler(rawdog, config, articles, article_dates):
	...
	for article in articles:
		try:
			values = [
						article.entry_info['link'] if 'link' in article.entry_info else "",
						article.feed, article.entry_info['title'] if 'title' in article.entry_info else "",
						article.entry_info['summary'] if 'summary' in article.entry_info else "",
						article.entry_info['author_detail']['email'] if 'author_detail' in article.entry_info else "",
						article.entry_info['author_detail']['name'] if 'author_detail' in article.entry_info else "",
						article.hash
					]
